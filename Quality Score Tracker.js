...
function trackQS(config) {
    ...
    for (var key in qsStats) {
        ...
        for (var i = 1; i <= 10; i++) {
            if (qsStats[key][0]) newValues.push([qsStats[key][i] / qsStats[key][0]]);
    ...
    for (var chartName in config['chartsToDisplay']) {
        ...
        sheetCharts.getRange(row + 1, 2, 2, 1).setFormulasR1C1(
            [
                ["=LOWER(Averages!R1C" + chartsProperties[chartName]['vCol'] + ")"],
                ["=Averages!R" + lastRow + "C" + chartsProperties[chartName]['vCol']]
...
function addReportFile(folder, name) {
    ...
    sheet.getRange(1, 1, 1, 4).setValues([
        ["Campaign", "AdGroup", "Keyword", "ID string"]
    ]);
...
function updateInfo(summarySpreadsheet, version) {
    ...
    if (versionHistory[0][0] != version) {
        ...
        newVersionSheet.getRange(1, 1, 6, 2).setValues([
            ["Latest version:", versionHistory[0][0]],
            ["Your version:", version],
            ["", ""],
            ["Get the latest version at", "http://www.ppc-epiphany.com/qstracker/latest"],
            ["", ""],
            ["Newer Versions", ""]
        ]);
